...
&emsp;&emsp;Gistable([6])介绍了Gistable数据集，它是从GitHub的公共gist服务中挖掘的10,259个单文件Python脚本的集合。通过查询GitHub gist在Python语言中搜索具有至少一个星评级的gist的UI，并自动抓取返回的结果，可以发现数据集中的gist。对Gistable数据集的分析表明，数据集中最常见的gist退出状态是导入错误，发生概率为52%。特别是，Gistable尝试安装一个包来命名不是在标准库中导入的每个资源，这种天真的方法无法修复2,891个gist中的导入错误。我们关注的是这些“硬指标”，而Gistable的朴素推断算法并不能满足这些指标，我们称之为HG2.9k。
&emsp;&emsp;答:方法
为了评估DockerizeMe，我们分析了它在HG2.9k中从gist中删除导入错误的能力。分析每个gist的第一步是使用DockerizeMe执行依赖项解析，以生成要安装的软件包列表。然后，我们在一个干净的Python 2.7.14中使用Python包管理器pip安装每个包p∈p。每个gist的执行结果被记录为运行时(如果有的话)或成功时抛出的异常的名称。我们认为，在依赖项解析的上下文中，如果它的退出状态不是输入恐怖，那么它的主旨应该得到修复。
在评估gist时遇到的安装失败被忽略。这种选择的基本原理是，如果失败的包是gist的直接依赖项或传递依赖项，那么执行将会引起极大的恐惧。但是，由于DockerizeMe知识库中的关联规则的性质，包不一定是真正的依赖项。如果是这样，可以继续不使用包。
所有的gist都在基于python:2.7.14图像的Docker容器中进行了评估。为了提高效率，DockerizeMe Docker映像将aptitude包管理器配置为通过本地代理，并通过禁用文件系统缓存和将默认超时设置为10分钟来配置pip。分析作业是在Nomad集群上调度的，Nomad集群是一个进程和容器管理系统，它本身就支持调度Docker容器。计算节点运行ubuntu16.04，有4个cpu和8GB内存。
b .结果
我们主要根据相应的gist是否经历了导入来对推断环境的质量进行排序 

&emsp;&emsp;错误执行期间。对于成功的推论，我们还将根据直接依赖项和传递依赖项的数量以及总体依赖项的总数来考虑推断环境的特征。然后，我们讨论在推断环境配置中遇到的安装故障，以及推断过程可能无法生成工作环境的原因。最后，我们将固定的gist的退出状态视为自动化环境配置中需要解决的未来问题的一个指示。
1)推理:我们评估DockerizeMe的推理过程共有2891从HG2.9k Python》成书,Gistable数据集的一个子集选择作为基准,不能固定安装每个包的资源名称,自然第一步由开发人员试图解决导入错误[6]。我们的评估发现，在安装Dock- erizeMe推理过程中发现的依赖项所生成的环境中，另外有892个gist(31%)在没有体验过Python的重要性的情况下执行。该结果说明了环境配置自动推理的一个重要步骤。我们将在下一节中讨论未来的改进。
2)环境特性:平均而言，DockerizeMe从HG2.9k中生成的工作环境规范导入了2 - 3个独特的资源，与HG2.9k中所有gist的总体平均值一致。DockerizeMe报告了对前79%的gist执行至少一个名称解析，这意味着大多数的gist导入一个资源，该资源被映射到一个具有不同名称的包。40%的人至少有一个传递依赖。总的来说，导入的资源平均映射到三个直接依赖项，另外两个传递依赖项由推理过程发现。传递依赖项最有可能是附加的Python包。
3)规模:DockerizeMe提出的最大的环境规范包含206个独特的Python和APT包。Python包在安装的包总数中占有较大的份额，一个环境中最多安装195个包，而APT包最多安装48个包。相比之下，在那些修复了gist导入错误的环境规范中，最大的环境规范只包含87个惟一的包，最多安装了80个Python包，9个APT包可以安装在任何环境中。
在这两种情况下，大的环境尺寸都是一个异常值，大多数工作配置都是大于17×小。最大工作配置的大小表明需要一个后推理减少过程，该过程能够将一组建议的包减少到最小，从而仍然解决脚本的导入错误。
4)安装失败的频率:我们的评估过程忽略了推断依赖项的安装错误，假设如果一个包安装失败，但整体配置仍然有效，则可以从最终配置中删除失败的包。在892个固定的gist中，只有9个(1%)在构建推断的环境时发生了安装错误，但是仍然退出了，没有发生导入错误。人工检查发现4处故障

是由于对Python包的未知依赖，而1是由于对系统包的未知依赖。其余的失败要么是由于包本身的问题，要么是由于网络噪音。
5)剩余的失败原因:在HG2.9k中对所有的gist进行推理后，1999个gist仍然退出了Python的ImportError。我们对30个这样的随机样本以及生成的环境规范执行了一个附加的定性编码过程，以确定推理为什么不能生成工作环境。
在执行编码时，检查每个gist以确定其退出状态的根本原因。然后我们重新运行DockerizeMe的推理算法，在运行时检查推理过程，以确定为什么根本原因没有得到修复。我们根据没有生成正确的环境规范的最终原因对每个gist进行编码。我们在编码过程中采用协商一致的方式来解决编码[17]的可靠性问题。使用这种技术，第一作者和第二作者合作澄清代码的定义，并就所分配的代码达成一致。
在分析的30个gist中，有15个出现了最常见的失败原因，这是因为环境规范没有发现直接的依赖关系。也就是说，gist在安装了所有推断的依赖项之后，导入了一些不存在的资源，这表明有一些从资源到DockerizeMe不知道的Python包的映射。由于环境推断没有找到传递依赖项，导致四个gist失败，这意味着推断依赖项由于额外的配置需求而无法安装，或者在推断环境中执行导致对gist没有直接导入的资源的导入错误。另外三个必需的gist为它们的依赖项推断出了正确的包，但是由于破坏了对包的API所做的更改，需要另一个版本。
其余的gist由于导入错误而失败，但是无法通过从PyPI或APT推断依赖项来修复。例如，一些gist需要系统CPython解释器之外的执行环境。其中一个要点是作为一个出色的文本插件编写的，需要在编辑器中运行。其他的gist导入了用户需要编写的本地配置文件。
...
6)固定的gist退出状态:表I为HG2.9k的gist退出状态，导入错误通过DockerizeMe解决。最常见的退出状态是成功，超过50%的紧急状态发生。下一个最常见的退出状态是Python的NameError，意思是要点试图访问不存在的对象引用。在大多数情况下，NameError实例是gist本身的问题，不在DockerizeMe的关注范围之内。一个例外是使用通配符导入，其中包提供的资源已经更改。对于未来的研究，我们可以通过静态分析来确定这个用例是否常见，是否需要寻址。

在剩下的30%中，91个退出是由于Django配置错误造成的，41个退出是由于IOError造成的。ImproperlyConfigured是Django框架在初始化时抛出的异常，如果它在设置文件中缺少所需的配置。IOError可以指示与服务(如数据库服务器)通信的问题。
应用推断后最常见的退出码与Gistable([6]表I)发现的非常相似，但SyntaxError及其子IndentationError除外，它们被选择条件过滤掉了。
未来的研究可以评估，为了生成提供这些资源的环境配置，gist如何依赖配置文件、环境变量和外部服务。
